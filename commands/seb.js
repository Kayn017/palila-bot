...
const tember = (message, args, conf) => {
	...
	message.channel.send(stocksEmbed).catch(error => err(`Impossible d'envoyer un message sur le channel.`, message, error));
}
...
const fact = async (message, args, conf) => {
	try {
		await message.channel.send(await _random_fact(message, conf));
	} catch (e) {
